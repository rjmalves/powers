...
impl UncertaintyModel {
    /// TEMPORARY: Convert to old UnifiedNoiseSpec format for backward compatibility
    /// This allows gradual migration from old to new API
    /// TODO: Remove after all code uses UncertaintyModel directly
    pub fn to_unified_noise_spec(
        &self,
        ...
        match self {
            UncertaintyModel::Independent {
                ...
                let mut seasonal_params_map = HashMap::new();
                for (season_id, params) in seasonal_params.iter().enumerate() {
                    let marginal_override = match params.distribution {
                        DistributionType::Normal => None,
                        DistributionType::LogNormal3 { gamma, mu, sigma } => {
                            Some(crate::input::MarginalDistribution::LogNormal3 {
                                gamma,
                                mu,
                                sigma,
                            })
                ...
                UnifiedNoiseSpec {
                    uncertainty_type: *entity_type,
                    entity_id: *entity_id,
                    temporal_model: TemporalModelSpec::Independent,
                    seasonal_params: seasonal_params_map,
                }
            }
            UncertaintyModel::PeriodicAR {
                ...
                for season_id in 0..par_params.num_seasons {
                    // AR parameters
                    seasonal_ar_params.insert(
                        season_id,
                        SeasonalPARParams {
                            ar_order: par_params.ar_orders[season_id],
                    ...
                    // Seasonal parameters
                    let marginal_override = match par_params.seasonal_distributions[season_id] {
                        DistributionType::Normal => None,
                        DistributionType::LogNormal3 { gamma, mu, sigma } => {
                            Some(crate::input::MarginalDistribution::LogNormal3 {
                                gamma,
                    ...
                    seasonal_params_map.insert(
                        season_id,
                        SeasonalNoiseParams {
                            mean: par_params.seasonal_means[season_id],
                            std_dev: par_params.seasonal_stds[season_id],
                            marginal_override,
                        },
                    );
                }
...
/// TEMPORARY: Convert Vec<UncertaintyModel> to Vec<UnifiedNoiseSpec> for backward compatibility
pub fn to_unified_noise_specs(
    models: &[UncertaintyModel],
) -> Vec<crate::unified_noise_spec::UnifiedNoiseSpec> {
    models.iter().map(|m| m.to_unified_noise_spec()).collect()
}
