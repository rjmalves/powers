...
class RssMonitor:
        ...
        while not self._stop_event.is_set():
            ...
            if elapsed - last_sample_time >= self.interval:
                ...
                if rss_kb is not None:
                    sample = RssSample(
                        timestamp=elapsed, rss_kb=rss_kb, wall_time=current_time
                    )
    ...
    def start_monitoring(self, pid: int):
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, args=(pid, start_time), daemon=True
        )
    ...
    def compute_summary(self) -> Dict[str, Any]:
        ...
        if len(self.samples) >= 10:
            first_half_mean = sum(rss_values[: len(rss_values) // 2]) / (
                len(rss_values) // 2
            )
            second_half_mean = sum(rss_values[len(rss_values) // 2 :]) / (
                len(rss_values) - len(rss_values) // 2
            )
    ...
    def collect(
        ...
        *,
        ...
        if not Path("/proc").exists():
            errors.append(
                "/proc filesystem not available; RSS monitoring requires Linux"
            )
        ...
        try:
            ...
            errors.append("Process timeout after 1800 seconds")
            process.kill()
            process.wait()
            exit_code = -1
        ...
        if summary["sample_count"] < 5:
            warnings.append(
                f"Only {summary['sample_count']} samples collected; process may have been too short"
            )

        if summary.get("growth_detected", False):
            warnings.append(
                f"Memory growth detected: {summary['peak_rss_mb']:.1f} MB peak"
            )
