...
def _parse_massif_out(massif_path: Path) -> Dict[str, Any]:
    ...
    if snapshots:
        ...
        summary["peak_heap_mb"] = peak_snapshot["mem_heap_bytes"] / (
            1024 * 1024
        )
        ...
        if len(snapshots) >= 2:
            ...
            growth_bytes = (
                final_snapshot["total_bytes"] - first_snapshot["total_bytes"]
            )
    ...
    summary["snapshots"] = snapshots[
        :10
    ]  # Include first 10 snapshots for detail
    ...
    def collect(
        ...
        *,
        ...
        rc, out, err = _run_command(
            massif_cmd, cwd=config.repo_root, timeout=1800
        )
