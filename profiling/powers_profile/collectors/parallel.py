...
from .contention import ContentionDetector
from .scaling_runner import ScalingTestConfig, ScalingTestRunner
    ...
    def collect(
        ...
        if enable_contention:
            if not self.contention_detector.is_available():
                print(
                    "⚠️  Contention detection requested but perf is not available"
                )
            else:
                ...
                for result in scaling_results:
                    if result.success:
                        metrics = self.contention_detector.collect(
                            binary, args, result.thread_count, timeout_seconds
                        )
                        contention_data.append(metrics)

                        if metrics.success:
                            # Attach to scaling result
                            result.perf_data = self.contention_detector.to_dict(
                                metrics
                            )
        ...
        try:
            ...
            print(
                f"✅ Computed speedup/efficiency for {len(speedup_metrics)} thread counts"
            )
        ...
        if len(speedup_metrics) >= 2:
            try:
                amdahl_estimate = estimate_amdahl_serial_fraction(
                    speedup_metrics, method="harmonic"
                )
                print(
                    f"📊 Amdahl estimate: {amdahl_estimate.serial_fraction * 100:.2f}% serial fraction"
                )
            except ValueError as e:
        ...
        # Detect bottlenecks
        bottlenecks = (
            detect_scaling_bottlenecks(speedup_metrics)
            if speedup_metrics
            else []
        )
        if bottlenecks:
