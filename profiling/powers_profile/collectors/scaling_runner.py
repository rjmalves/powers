...
class ScalingTestRunner:
    ...
    def __init__(self, config: ProfilingConfig, scaling_config: ScalingTestConfig):
    ...
    def _validate_config(self) -> None:
        ...
        if len(self.scaling_config.thread_counts) != len(set(self.scaling_config.thread_counts)):
    ...
    def run(self, binary: Path, args: List[str]) -> List[ScalingRunResult]:
        ...
        for thread_count in self.scaling_config.thread_counts:
            try:
                result = self._run_single_thread_count(binary, args, thread_count)
                results.append(result)
                
                if not result.success and not self.scaling_config.continue_on_error:
                    print(f"⚠️  Stopping scaling test due to failure at {thread_count} threads")
                    break
                ...
                if not self.scaling_config.continue_on_error:
                    print(f"⚠️  Stopping scaling test due to exception at {thread_count} threads: {e}")
                    break
        ...
        for i in range(self.scaling_config.measurement_iterations):
            try:
                ...
                return ScalingRunResult(
                    ...
                    std_dev=self._compute_std_dev(durations) if durations else 0.0,
