...
fn test_forward_pass_state_initialization() {
    ...
    let result = sddp.train(1, 1, false, &saa, false, false, None);
...
fn test_forward_pass_uncertainty_realization() {
    ...
    let result1 = sddp
        .train(1, 1, false, &saa, false, false, None)
    ...
    let result2 = sddp2
        .train(1, 1, false, &saa2, false, false, None)
...
fn test_forward_pass_state_continuity() {
    ...
    // Run training
    let result = sddp.train(2, 1, false, &saa, false, false, None);
    assert!(
        result.is_ok(),
...
fn test_forward_pass_multiple_scenarios() {
    ...
    let result = sddp
        .train(2, 5, false, &saa, false, false, None)
