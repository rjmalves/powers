...
> **Dependencies**: [T-025](./ticket-025-implement-coordinator.md)
...
- `src/algorithm/coordinator.rs` - `ParallelHandlerCoordinator` (from T-025)
...
- [ ] `ParallelHandlerCoordinator` created in `train()`
...
Find all `train_handlers` and replace with `coordinator.handlers_mut()` where mutable access is needed.
...
- ⚠️ Keep FCF reference valid for coordinator lifetime
